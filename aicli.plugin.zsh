# Configuration
: ${AI_CLI_MODEL:="qwen2.5-coder:1.5b"}
: ${AI_CLI_ENDPOINT:="http://localhost:11434/api/chat"}
: ${AI_CLI_DEBOUNCE:=0.5}

# Check dependencies
if ! command -v curl >/dev/null 2>&1 || ! command -v jq >/dev/null 2>&1; then
    echo "aicli: curl and jq are required."
    return
fi

# History management
_history_file="$HOME/.aicli_history.json"

_read_history() {
  if [[ -f "$_history_file" ]]; then
    cat "$_history_file"
  else
    echo '[]'
  fi
}

_write_history() {
  local content="$1"
  # Validate JSON before writing to avoid corruption
  if jq empty <<<"$content" &>/dev/null; then
    echo "$content" > "$_history_file"
  else
    echo "aicli: Error - invalid JSON when saving history."
  fi
}

_clear_history() {
  _write_history '[]'
}

# Helper function to call Ollama with messages array
_aicli_call_llm_messages() {
    local messages_json="$1"

    # Construct JSON payload
    local json_payload=$(jq -n \
        --arg model "$AI_CLI_MODEL" \
        --argjson messages "$messages_json" \
        '{model: $model, messages: $messages, stream: false}')

    # Call curl
    local response
    response=$(curl -s -X POST "$AI_CLI_ENDPOINT" \
        -H "Content-Type: application/json" \
        -d "$json_payload" 2>&1) # Capture stderr
    local curl_exit=$?

    if [[ $curl_exit -ne 0 ]]; then
        echo "aicli: Connection failed (curl exit code $curl_exit). Is Ollama running at $AI_CLI_ENDPOINT?"
        return
    fi

    if [[ -z "$response" ]]; then
        echo "aicli: Empty response from server."
        return
    fi

    # Check for Ollama error
    local error_msg=$(print -r -- "$response" | jq -r '.error // empty' 2>/dev/null)
    if [[ -n "$error_msg" ]]; then
        echo "aicli error: $error_msg"
        return
    fi

    # Extract content
    local content=$(print -r -- "$response" | jq -r '.message.content' 2>/dev/null)

    if [[ -z "$content" ]] || [[ "$content" == "null" ]]; then
         # Fallback: print raw response if parsing failed but no explicit error
         # This helps debugging malformed JSON or unexpected format
         echo "aicli: Failed to parse response. Raw output:"
         print -r -- "$response" | jq . 2>/dev/null || print -r -- "$response"
    else
         print -r -- "$content"
    fi
}

# Feature 2: Quick Chat Query
# New chat function for ? command
_aicli_newchat() {
    if [[ $# -eq 0 ]]; then
        echo "Usage: ? <query>"
        return 1
    fi

    local query="$*"
    local context_pwd="$PWD"

    local system_prompt="You are a concise terminal expert. Provide the exact command(s) for: $query. Include brief flag explanations if helpful. Output only commands and short notes—no chit-chat. Current directory: $context_pwd"

    # Clear history for new conversation
    _clear_history

    # Build messages array with system prompt + user query
    local messages_json=$(jq -n \
        --arg sys "$system_prompt" \
        --arg q "$query" \
        '[{"role": "system", "content": $sys}, {"role": "user", "content": $q}]')

    # Call model with messages array
    printf "Querying AI..."
    local result=$(_aicli_call_llm_messages "$messages_json")
    
    printf "\r\033[K" # Clear current line
    print -r -- "$result"

    # Save to history: user and assistant messages (but not system)
    local new_history=$(jq --arg q "$query" --arg r "$result" '[
        {"role": "user", "content": $q},
        {"role": "assistant", "content": $r}
    ]' <<<"$messages_json")

    _write_history "$new_history"
}

# Follow-up function for ?? command
_aicli_followup() {
    if [[ $# -eq 0 ]]; then
        echo "Usage: ?? <query>"
        return 1
    fi

    local query="$*"
    local context_pwd="$PWD"

    local system_prompt="You are a concise terminal expert. Provide the exact command(s) for: $query. Include brief flag explanations if helpful. Output only commands and short notes—no chit-chat. Current directory: $context_pwd"

    # Read current history
    local current_history=$(_read_history)

    # Build messages array:
    # [system_prompt, all current history items (user/assistant), new user query]
    local messages_json=$(jq -n \
        --arg sys "$system_prompt" \
        --argjson hist "$current_history" \
        --arg q "$query" \
        '[{"role": "system", "content": $sys}] + $hist + [{"role": "user", "content": $q}]')

    # Call model with messages array
    printf "Querying AI..."
    local result=$(_aicli_call_llm_messages "$messages_json")
    
    printf "\r\033[K" # Clear current line
    print -r -- "$result"

    # Add new user and assistant to history
    local new_history=$(echo "$current_history" | jq --arg q "$query" --arg r "$result" '. + [{"role": "user", "content": $q}, {"role": "assistant", "content": $r}]')
    
    _write_history "$new_history"
}

# Aliases for new commands
alias \?='noglob _aicli_newchat'
alias '??'='noglob _aicli_followup'


# Feature 3: Command Explanation
_aicli_explain_widget() {
    local buffer_content="$BUFFER"
    if [[ -z "$buffer_content" ]]; then
        zle -M "No command to explain."
        return
    fi

    zle -M "Asking AI to explain..."

    local system_prompt="Explain this command briefly, including what each flag does. Be concise."
    local messages_json=$(jq -n \
        --arg sys "$system_prompt" \
        --arg q "$buffer_content" \
        '[{"role": "system", "content": $sys}, {"role": "user", "content": $q}]')
    local explanation=$(_aicli_call_llm_messages "$messages_json")

    zle -M "$explanation"
}

zle -N _aicli_explain_widget
bindkey '^X' _aicli_explain_widget

# Feature 1: AI-Powered Inline Autosuggestion (using zle -F)

_AICLI_FD=0
_AICLI_LAST_BUFFER=""

# Async worker logic
_aicli_fetch_suggestion_worker() {
    local buffer="$1"
    local pwd="$2"

    # Debounce
    sleep "$AI_CLI_DEBOUNCE"

    local system_prompt="You are a Linux/macOS terminal expert. Complete the partial command intelligently, considering pwd and common flags. Output only the full suggested command line (no explanations). Current directory: $pwd"

    local messages_json=$(jq -n \
        --arg sys "$system_prompt" \
        --arg q "$buffer" \
        '[{"role": "system", "content": $sys}, {"role": "user", "content": $q}]')
    local suggestion=$(_aicli_call_llm_messages "$messages_json")

    if [[ -n "$suggestion" ]]; then
        print -r -- "$suggestion"
    fi
}

# Handler for data on FD
_aicli_zle_handler() {
    local fd=$1
    local suggestion

    # Read all content from the file descriptor
    # Using cat <&$fd is reliable for pipes/substitutions
    suggestion=$(cat <&$fd)

    # Close FD in ZLE (stops watching)
    zle -F "$fd"
    exec {fd}<&-
    _AICLI_FD=0

    # Verify suggestion starts with BUFFER
    if [[ "$suggestion" == "$BUFFER"* ]]; then
            local suffix="${suggestion#$BUFFER}"
            if [[ -n "$suffix" ]]; then
                # Gray color for suggestion
                POSTDISPLAY=$'\x1b[90m'"$suffix"$'\x1b[0m'
                zle -R
            fi
    fi
}

# Trigger function
_aicli_trigger_autosuggest() {
    # Skip if buffer empty or same as last time
    if [[ -z "$BUFFER" ]] || [[ "$BUFFER" == "$_AICLI_LAST_BUFFER" ]]; then
        return
    fi
    _AICLI_LAST_BUFFER="$BUFFER"

    # Cancel previous async job if needed
    if [[ $_AICLI_FD -ne 0 ]]; then
        zle -F "$_AICLI_FD" # Stop watching
        exec {_AICLI_FD}<&- # Close
        _AICLI_FD=0
    fi

    # Clear current suggestion
    POSTDISPLAY=""

    # Start worker using process substitution
    # We silence stderr of the worker to avoid leaking errors to the prompt
    exec {_AICLI_FD}< <(_aicli_fetch_suggestion_worker "$BUFFER" "$PWD" 2>/dev/null)

    # Watch FD
    zle -F "$_AICLI_FD" _aicli_zle_handler
}

# Hook into ZLE non-destructively
autoload -Uz add-zle-hook-widget

# Hook `zle-line-pre-redraw`
_aicli_pre_redraw() {
    _aicli_trigger_autosuggest
}

if zle -N _aicli_pre_redraw; then
     add-zle-hook-widget zle-line-pre-redraw _aicli_pre_redraw
fi

# Accept suggestion
_aicli_accept_suggestion() {
    if [[ -n "$POSTDISPLAY" ]]; then
        local clean_suggestion
        # Use perl for portable ANSI stripping (handles macOS/Linux better than sed \x1b)
        if command -v perl >/dev/null 2>&1; then
            clean_suggestion=$(print -r -- "$POSTDISPLAY" | perl -pe 's/\e\[[0-9;]*m//g')
        else
            # Fallback to sed with printf for portability
             clean_suggestion=$(print -r -- "$POSTDISPLAY" | sed "s/$(printf '\033')\[[0-9;]*m//g")
        fi

        BUFFER="${BUFFER}${clean_suggestion}"
        POSTDISPLAY=""
        CURSOR=$#BUFFER
    else
        zle .forward-char
    fi
}
zle -N _aicli_accept_suggestion

# Bindings
bindkey '^[[C' _aicli_accept_suggestion
bindkey '^F' _aicli_accept_suggestion
